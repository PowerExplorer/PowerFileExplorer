...
public class RootHelper {
     ...
     * thread {@link ExplorerActivity#handlerThread} thus any code run in callback must be thread safe.
    ...
    public static ArrayList<String> runShellCommand(String cmd) throws RootNotPermittedException {
        if (ExplorerActivity.shellInteractive == null || !ExplorerActivity.shellInteractive.isRunning())
            throw new RootNotPermittedException();
        final ArrayList<String> result = new ArrayList<>();

        // callback being called on a background handler thread
        ExplorerActivity.shellInteractive.addCommand(cmd, 0, new Shell.OnCommandResultListener() {
            @Override
        ...
        ExplorerActivity.shellInteractive.waitForIdle();
        return result;
    }
    ...
    public static void runShellCommand(String cmd, Shell.OnCommandResultListener callback)
        ...
        if (ExplorerActivity.shellInteractive == null || !ExplorerActivity.shellInteractive.isRunning())
            throw new RootNotPermittedException();
        ExplorerActivity.shellInteractive.addCommand(cmd, 0, callback);
        ExplorerActivity.shellInteractive.waitForIdle();
    }
                ...
                if (ls != null) {
                    for (int i = 0; i < ls.size(); i++) {
                        ...
                        if (!file.contains("Permission denied"))
                            try {
                                BaseFile baseFile = Futils.parseName(file);
                                baseFile.setMode(OpenMode.ROOT);
                                if (baseFile != null) {
                                    baseFile.setName(baseFile.getPath());
                                    baseFile.setPath(path + "/" + baseFile.getPath());
                                    if (baseFile.getLink().trim().length() > 0) {
                                        boolean isdirectory = isDirectory(baseFile.getLink(), root, 0);
                                        baseFile.setDirectory(isdirectory);
                                    } else baseFile.setDirectory(isDirectory(baseFile));
                                    files.add(baseFile);
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
